$(document).ready(() => {
  ...
  function fetchNodes(arr) {
    ...
    if (arr !== null) {
      arr.forEach((elem) => {
        if (!nodeObj[elem]) {
          const newNode = new CardNode(null, elem, fetchNodes(logicArr[elem]));
  ...
  function uncover(board) {
    ...
    for (let i = 0; i < tierFour.length; i++) {
      const tierFourCard = nodeObj[tierFour[i]].data;
      if (!reseted) {
        tierFourCard.flip();
      }
      const $card = $('<img></img>');
  ...
  function popPyramids(cardArr, tier) {
    const cardsPerPyramid = cardArr.length / 3;
    const image = null;
